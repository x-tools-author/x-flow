 ...
 *
 * The file is encoded in "utf8 with bom", it is a part of xFlow project.
 *
 * xFlow is licensed according to the terms in the file LICENCE(GPL V3) in the root of the source code
 * directory.
 **************************************************************************************************/
#include "common.h"
...
void setupTextFormat(QComboBox *comboBox)
{
    ...
    comboBox->setCurrentIndex(comboBox->findData(static_cast<int>(TextFormat::Ascii)));
}
...
void setupAddition(QComboBox *comboBox)
{
    ...
    comboBox->setCurrentIndex(comboBox->findData(static_cast<int>(Affixes::None)));
}
...
void setupEscapeCharacter(QComboBox *comboBox)
{
    ...
    comboBox->setCurrentIndex(comboBox->findData(static_cast<int>(EscapeCharacter::None)));
}
...
QJsonObject saveSocketItem(const SocketItem &context)
{
    ...
    parameters.insert(keys.serverAddress, context.serverAddress);
    parameters.insert(keys.serverPort, context.serverPort);
    parameters.insert(keys.dataChannel, static_cast<int>(context.dataChannel));
    parameters.insert(keys.authentication, context.authentication);
    parameters.insert(keys.username, context.username);
    parameters.insert(keys.password, context.password);
    parameters.insert(keys.multicastAddress, context.multicastAddress);
    parameters.insert(keys.multicastPort, context.multicastPort);
    parameters.insert(keys.enableMulticast, context.enableMulticast);
    parameters.insert(keys.justMulticast, context.justMulticast);
