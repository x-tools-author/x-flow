...
QObject *SerialPort::initDevice()
{
    ...
    SerialPortItem item = loadSerialPortItem(parameters);
    if (item.portName.isEmpty()) {
        return nullptr;
    }
    ...
    if (!m_serialPort->open(QIODevice::ReadWrite)) {
        QString info = tr("Failed to open serial port: %1").arg(m_serialPort->errorString());
...
void SerialPort::readBytesFromDevice()
{
    ...
    QJsonObject tmp = save();
    SerialPortItem item = loadSerialPortItem(tmp);
