...
void WebSocketServer::setupSocket(QWebSocket *socket)
{
    ...
    connect(socket, &QWebSocket::disconnected, socket, [=, this]() { removeClient(flag); });
    connect(socket, &QWebSocket::disconnected, socket, [=, this]() { this->m_sockets.removeAll(socket); });
    ...
    connect(socket, xWebSocketErrorOccurred, socket, [=, this]() {
