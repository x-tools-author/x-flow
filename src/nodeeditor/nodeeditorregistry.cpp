...
void NodeEditorRegistry::registerModelCommunication()
{
    ...
    for (int &type : deviceTypes) {
        // clang-format off
        registerModel<Communication>([=, this]() { return std::make_unique<Communication>(type); }, category);
        // clang-format on
...
void NodeEditorRegistry::registerModelHub()
{
    ...
    for (Ctx &ctx : list) {
        // clang-format off
        registerModel<Hub>([=, this]() { return std::make_unique<Hub>(ctx.inPorts, ctx.outPorts); }, category);
        // clang-format on
...
void NodeEditorRegistry::registerModelPlugin()
{
    ...
    for (PluginManager::PluginContext &pluginCtx : pluginCtxs) {
        // clang-format off
        registerModel<PluginNode>([=, this]() { return std::make_unique<PluginNode>(pluginCtx.libFile); }, category);
        // clang-format on
